...
// Set move/cut word delimiter, defaut is all not digital and alphabetic characters.
	...
	int crossline_match_patterns (const char *str, char *word[], int num) {
		int i;
		char buf[CROSS_HISTORY_BUF_LEN];
	...
	int crossline_split_patterns (char *patterns, char *pat_list[], int max) {
		int i, num = 0;
		char *pch = patterns;
	...
	int crossline_history_dump (FILE *file, int print_id, char *patterns, int sel_id, int paging) {
		uint32_t i;
		int		id = 0, num=0;
	...
	int crossline_history_search (char *input) {
	...
	void crossline_refreash (const char *prompt, char *buf, int *pCurPos, int *pCurNum, int new_pos, int new_num, int bChg) {
	...
	void crossline_print (const char *prompt, char *buf, int *pCurPos, int *pCurNum, int new_pos, int new_num) {
		*pCurPos = *pCurNum = 0;
		crossline_refreash (prompt, buf, pCurPos, pCurNum, new_pos, new_num, 1);
	}

	// Copy part text[cut_beg, cut_end] from src to dest
	void crossline_text_copy (char *dest, const char *src, int cut_beg, int cut_end) {
	...
	void crossline_history_copy (const char *prompt, char *buf, int size, int *pos, int *num, int history_id) {
	...
	int crossline_key_esc2alt (int ch) {
		switch (ch) {
	...
	int crossline_key_mapping (int ch) {
		switch (ch) {
	#ifndef _WIN32
	...
	int crossline_getkey (int *is_esc) {
	...
	int crossline_get_esckey (int ch) {
		int ch2;
